...
router.get('/', asyncHandler(async (req, res) => {
  ...
  const result = await carbonService.listElectricityInvoices({
    companyId,
...
router.post('/', asyncHandler(async (req, res) => {
  ...
  try {
    const invoice = await carbonService.createElectricityInvoice({
      companyId,
      userId: req.userId,
      payload: req.body
    });
...
router.put('/:id', asyncHandler(async (req, res) => {
  ...
  const invoice = await carbonService.updateElectricityInvoice({
    id: req.params.id,
    companyId,
    changes: req.body
  });
  if (!invoice) {
    return sendError(res, {
      status: 404,
      code: 'NOT_FOUND',
      message: 'Electricity invoice not found.'
    });
  }
...
router.delete('/:id', asyncHandler(async (req, res) => {
  ...
  const deleted = await carbonService.deleteElectricityInvoice({
    id: req.params.id,
    companyId
  });
  if (!deleted) {
