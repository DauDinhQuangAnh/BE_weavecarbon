...
async function generateProductCarbonPdf(doc, companyId, database = pool) {
...
async function generateBatchExportPdf(doc, companyId, database = pool) {
  const client = await database.connect();
  try {
    const [companyRes, shipmentsRes] = await Promise.all([
      ...
      client.query(`
        ...
        LEFT JOIN latest_product_assessment_snapshots ps
          ON ps.product_id = sp.product_id AND ps.company_id = s.company_id
...
async function generateFacilityEmissionPdf(doc, companyId, database = pool) {
  const client = await database.connect();
  try {
...
async function generateCompliancePdf(doc, companyId, database = pool) {
  const client = await database.connect();
  try {
 ...
 */
async function generatePdf(reportType, companyId, database = pool) {
  ...
  const recordCount = await generator(doc, companyId, database);
  doc.end();
  await endPromise;
...
const createPdfReportService = ({ database = pool } = {}) => ({
  generatePdf: (reportType, companyId) => generatePdf(reportType, companyId, database)
});

module.exports = createPdfReportService();
module.exports.generatePdf = generatePdf;
module.exports.createPdfReportService = createPdfReportService;
