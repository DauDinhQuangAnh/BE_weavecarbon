...
const REQUEST_BODY_OVERRIDES = {
  ...
  'POST /industry-packs/pilots': { type:'object',required:['packId','facilityRevisionId','processRevisionId','studyReference','periodStart','periodEnd','outputTonnes','productReference','allocation','methodology','productionEvidenceDocumentIds','activityLines'],properties:{packId:{type:'string',enum:['steel','cement']},facilityRevisionId:{type:'string',format:'uuid'},processRevisionId:{type:'string',format:'uuid'},studyReference:{type:'string',minLength:1,maxLength:120},periodStart:{type:'string',format:'date'},periodEnd:{type:'string',format:'date'},outputTonnes:{type:'number',minimum:0,exclusiveMinimum:true},productReference:{type:'string',minLength:1},allocation:{type:'object',required:['method','share'],properties:{method:{type:'string',enum:['single_product']},share:{type:'number',enum:[1]}},additionalProperties:false},methodology:{type:'object',required:['boundary','source'],properties:{boundary:{type:'string',minLength:1},source:{type:'string',minLength:1}},additionalProperties:false},productionEvidenceDocumentIds:{type:'array',minItems:1,uniqueItems:true,items:{type:'string',format:'uuid'}},activityLines:{type:'array',minItems:3,items:{type:'object',required:['category','sourceReference','activityQuantity','activityUnit','factorProposalId','evidenceDocumentIds'],properties:{category:{type:'string',enum:['process','fuel','electricity']},sourceReference:{type:'string',minLength:1},activityQuantity:{type:'number',minimum:0},activityUnit:{type:'string',minLength:1},factorProposalId:{type:'string',format:'uuid'},evidenceDocumentIds:{type:'array',minItems:1,items:{type:'string',format:'uuid'}}},additionalProperties:false}}},additionalProperties:false },
