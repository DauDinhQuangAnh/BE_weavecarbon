...
const REQUEST_BODY_OVERRIDES = {
  ...
  'POST /vn-mrv/cases': { type: 'object', required: ['caseReference','facilityRevisionId','reportingYear','sector','applicabilityStatus','assessmentDate','rationale'], properties: {
    caseReference:{type:'string',minLength:1,maxLength:120},facilityRevisionId:{type:'string',format:'uuid'},reportingYear:{type:'integer',minimum:2020,maximum:2200},sector:{type:'string',enum:['energy','industry_trade','transport','construction','agriculture_environment','waste','other']},applicabilityStatus:{type:'string',enum:['confirmed_listed','potentially_listed','not_listed','undetermined']},listingReference:{type:'string'},listingEvidenceDocumentId:{type:'string',format:'uuid',nullable:true},assessmentDate:{type:'string',format:'date'},rationale:{type:'string',minLength:1,maxLength:5000}},additionalProperties:false },
  'POST /vn-mrv/measurement-plans': { type:'object',required:['caseId','planReference','organizationalBoundary','operationalBoundary','sourceMap','methodology','qaqcPlan','uncertaintyPlan','dqlAssessmentIds','evidenceDocumentIds'],properties:{caseId:{type:'string',format:'uuid'},planReference:{type:'string',minLength:1,maxLength:120},organizationalBoundary:{type:'object'},operationalBoundary:{type:'object'},sourceMap:{type:'array',minItems:1,items:{type:'object'}},methodology:{type:'object'},qaqcPlan:{type:'object'},uncertaintyPlan:{type:'object'},dqlAssessmentIds:{type:'array',items:{type:'string',format:'uuid'}},evidenceDocumentIds:{type:'array',items:{type:'string',format:'uuid'}}},additionalProperties:false },
  'POST /vn-mrv/filings': { type:'object',required:['caseId','measurementPlanId','corporateInventoryId'],properties:{caseId:{type:'string',format:'uuid'},measurementPlanId:{type:'string',format:'uuid'},corporateInventoryId:{type:'string',format:'uuid'}},additionalProperties:false },
  'POST /mitigation-operations/initiatives': { type:'object',required:['initiativeReference','facilityRevisionId','title','lifecycleStatus','ownerName','baselineYear','targetReductionTco2e','plannedStart','plannedEnd','methodology','assumptions','evidenceDocumentIds'],properties:{initiativeReference:{type:'string',minLength:1,maxLength:120},facilityRevisionId:{type:'string',format:'uuid'},title:{type:'string',minLength:1,maxLength:240},lifecycleStatus:{type:'string',enum:['proposed','approved_internal','in_progress','completed','cancelled']},ownerName:{type:'string',minLength:1,maxLength:240},baselineYear:{type:'integer',minimum:2020,maximum:2200},baselineInventoryId:{type:'string',format:'uuid',nullable:true},targetReductionTco2e:{type:'number',minimum:0,exclusiveMinimum:true},plannedStart:{type:'string',format:'date'},plannedEnd:{type:'string',format:'date'},methodology:{type:'object'},assumptions:{type:'object'},evidenceDocumentIds:{type:'array',minItems:1,uniqueItems:true,items:{type:'string',format:'uuid'}},evidenceRole:{type:'string',enum:['baseline','methodology','approval','implementation','monitoring','verification']}},additionalProperties:false },
  'POST /mitigation-operations/scenarios': { type:'object',required:['initiativeId','scenarioReference','scenarioType','periodStart','periodEnd','baselineEmissionsTco2e','projectedEmissionsTco2e','annualProjection','assumptions','sensitivity','evidenceDocumentIds'],properties:{initiativeId:{type:'string',format:'uuid'},scenarioReference:{type:'string',minLength:1,maxLength:120},scenarioType:{type:'string',enum:['baseline','planned','conservative','stress']},periodStart:{type:'string',format:'date'},periodEnd:{type:'string',format:'date'},baselineEmissionsTco2e:{type:'number',minimum:0},projectedEmissionsTco2e:{type:'number',minimum:0},annualProjection:{type:'array',minItems:1,items:{type:'object'}},assumptions:{type:'object'},sensitivity:{type:'object'},evidenceDocumentIds:{type:'array',minItems:1,uniqueItems:true,items:{type:'string',format:'uuid'}}},additionalProperties:false },
  'POST /mitigation-operations/allocations': { type:'object',required:['allocationReference','facilityRevisionId','reportingYear','instrumentType','recordStatus','quantityTco2e','notes'],properties:{allocationReference:{type:'string',minLength:1,maxLength:120},facilityRevisionId:{type:'string',format:'uuid'},reportingYear:{type:'integer',minimum:2020,maximum:2200},instrumentType:{type:'string',enum:['authority_quota','internal_budget','transfer_reference','credit_reference']},recordStatus:{type:'string',enum:['draft_reference','evidence_confirmed']},quantityTco2e:{type:'number',minimum:0,exclusiveMinimum:true},vintageYear:{type:'integer',minimum:2020,maximum:2200,nullable:true},externalReference:{type:'string',nullable:true},evidenceDocumentId:{type:'string',format:'uuid',nullable:true},notes:{type:'string',minLength:1,maxLength:5000}},additionalProperties:false },
  'POST /mitigation-operations/positions': { type:'object',required:['facilityRevisionId','corporateInventoryId','reportingYear','allocationIds','scenarioIds'],properties:{facilityRevisionId:{type:'string',format:'uuid'},corporateInventoryId:{type:'string',format:'uuid'},reportingYear:{type:'integer',minimum:2020,maximum:2200},allocationIds:{type:'array',uniqueItems:true,items:{type:'string',format:'uuid'}},scenarioIds:{type:'array',uniqueItems:true,items:{type:'string',format:'uuid'}}},additionalProperties:false },
  'POST /industry-packs/pilots': {
    type: 'object',
    required: ['packId','facilityRevisionId','processRevisionId','studyReference','periodStart','periodEnd','outputTonnes',
      'productReference','allocation','methodology','productionEvidenceDocumentIds','activityLines'],
    properties: {
      packId: { type: 'string', enum: ['steel','cement','textile_apparel','aluminium','construction_materials','fertiliser_chemicals','mining_minerals'] },
      facilityRevisionId: { type: 'string', format: 'uuid' }, processRevisionId: { type: 'string', format: 'uuid' },
      studyReference: { type: 'string', minLength: 1, maxLength: 120 }, periodStart: { type: 'string', format: 'date' },
      periodEnd: { type: 'string', format: 'date' }, outputTonnes: { type: 'number', minimum: 0, exclusiveMinimum: true },
      productReference: { type: 'string', minLength: 1, maxLength: 240 }, sectorContext: { type: 'object', maxProperties: 3, additionalProperties: true },
      allocation: { type: 'object', required: ['method','share'], properties: {
        method: { type: 'string', enum: ['single_product'] }, share: { type: 'number', enum: [1] }
      }, additionalProperties: false },
      methodology: { type: 'object', required: ['boundary','source'], properties: {
        boundary: { type: 'string', minLength: 1, maxLength: 4000 }, source: { type: 'string', minLength: 1, maxLength: 500 }
      }, additionalProperties: false },
      productionEvidenceDocumentIds: { type: 'array', minItems: 1, maxItems: 100, uniqueItems: true, items: { type: 'string', format: 'uuid' } },
      activityLines: { type: 'array', minItems: 3, maxItems: 100, items: { type: 'object',
        required: ['category','sourceReference','activityQuantity','activityUnit','factorProposalId','evidenceDocumentIds'],
        properties: { category: { type: 'string', enum: ['process','fuel','electricity','material','feedstock','steam','transport','waste','water'] },
          sourceReference: { type: 'string', minLength: 1, maxLength: 240 }, activityQuantity: { type: 'number', minimum: 0 },
          activityUnit: { type: 'string', minLength: 1, maxLength: 100 }, factorProposalId: { type: 'string', format: 'uuid' },
          evidenceDocumentIds: { type: 'array', minItems: 1, maxItems: 100, uniqueItems: true, items: { type: 'string', format: 'uuid' } }
        }, additionalProperties: false }
      }
    }, additionalProperties: false
