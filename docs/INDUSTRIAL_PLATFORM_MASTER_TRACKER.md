...
| G2-09 | Governed AI/OCR field review | Implemented software baseline | OCR output stays non-authoritative until a named human confirms every field against the exact evidence and extraction checksums; the downstream semantic/promotion continuation is delivered in G2-14 |
...
| G2-14 | Controlled AI/OCR activity promotion | Implemented software baseline | Deterministic versioned semantic suggestions, factor/calculation exclusion, exact human mapping decisions, anomaly/evidence-match review, immutable blocked/ready candidates and separate named promotion into canonical activity records implemented; representative-document/model evaluation and real-facility acceptance remain |
...
- Governed OCR-to-activity promotion that fails closed on unreviewed fields, preserves blocked candidates, caps OCR quality at L1-L3 and requires a second named administrator action before authoritative activity creation.

This baseline does **not** claim representative-document AI accuracy, independent verification from OCR, authority submission, registry ownership/transfer/surrender, expert-approved Industry Packs, production-grade WeaveNode telemetry, validated physical climate-risk forecasts, generic live SSO interoperability, penetration-test acceptance or production release acceptance.
