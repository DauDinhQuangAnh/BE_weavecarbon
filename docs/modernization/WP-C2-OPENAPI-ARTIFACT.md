...
The runtime-derived OpenAPI contract now has a deterministic, versioned JSON artifact at `openapi/openapi.json`. The JSDoc glob is normalized across Windows/Linux and object keys are canonicalized before serialization, so both platforms produce identical bytes. The artifact is the backend source consumed by frontend transport type generation.
