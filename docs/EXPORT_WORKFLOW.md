...
Commercial invoices require payment terms and exporter tax identity. Freight and insurance
are required when the selected Incoterm makes them relevant. Packing lists require marks,
dimensions, weights and a complete package-to-line quantity allocation. These values are
reconciled before a file can become `ready`.
...
2. Apply migration `017_shipment_export_workflow.sql` in staging.
